...
public:
    Arena(std::size_t capacity) {
        this->buffers_.push_back(Buffer{static_cast<char *>(::operator new(capacity)), capacity});
        this->offset_ = 0;
        this->bufferIdx_ = 0;
    ...
    void *allocate(std::size_t size, std::size_t alignment) {
        ...
        // Is usually buffer.back(), unless we did a reset and are left with a bunch of free buffers
        Buffer &current_buffer = *std::next(this->buffers_.begin(), this->bufferIdx_);
        ...
        if (std::align(alignment, size, aligned_ptr, space_left) == nullptr) {
            try {
                ...
                if (std::next(this->buffers_.begin(), this->bufferIdx_ + 1) == this->buffers_.end()) {

                    // We want to heap allocate as little as possible, so we double the capacity of each new buffer
                    std::size_t new_capacity = std::max(current_buffer.capacity * 2, size);
                    this->buffers_.push_back(Buffer{static_cast<char *>(::operator new(new_capacity)), new_capacity});
                    logger.log(Logger::LogLevel::DEBUG, "Created new buffer of capacity %lu at %p", new_capacity, &this->buffers_.back().data);
                }
                ...
                logger.log(Logger::LogLevel::ERROR, "Arena.allocate(): Failed to align, memory is likely full");
                throw std::bad_alloc();
            }
        }
    ...
    template <typename T, typename... Args> T *make(Args &&...args) {
        ...
        logger.log(Logger::LogLevel::DEBUG, "Allocated new object of size %lu and align %lu", sizeof(T), alignof(T));
